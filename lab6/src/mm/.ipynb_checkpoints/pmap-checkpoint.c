...
void mips_detect_memory()
{
    ...
    npage = basemem >> PGSHIFT; // basemem >> 12
    extmem = 0; // ???
...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
    ...
    pgtable = (Pte *)KADDR(PTE_ADDR(*pgdir_entryp)); //要转化为虚拟地址
    /* Step 2: If the corresponding page table is not exist and parameter `create`
     * is set, create one. And set the correct permission bits for this new page
     * table. */
    /* 要用 PTE_V 标志位判断页表是否存在, 注意是看页表是否存在是看页目录项 */
    if( !((*pgdir_entryp) & PTE_V) ) {
        if(create == 1) {
            pgtable = (Pte *)alloc(BY2PG, BY2PG, 1);// 这里得到的也是虚拟地址
            //(*pgdir_entryp) &= 0xFFF;//高20位清零,低12位其他属性保持不变
            (*pgdir_entryp) = PADDR((u_long)pgtable);
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    ...
    for(i = 0; i < size; i += BY2PG) {
        pgtable_entry = boot_pgdir_walk(pgdir, va + i, 1); //得到一个页表项指针
        /*物理地址pa是4KB的整数倍(低12位为0)，每次增加BY2PG = 4KB，那么就相当于pa的最后一位加1*/
        *pgtable_entry = pa + i;// 每个页表项保存的都是实际物理地址
...
page_init(void)
{
     ...
     * filed to 1) */
    /* freemem(这是个虚拟地址)地址以下的部分都已经使用过了，即这个虚拟地址以下对应的物理页面已经使用
     * pages数组管理着这些实际物理页面，因此对应着要修改page数组
     */
...
page_alloc(struct Page **pp)
{
    ...
    *pp = ppage_temp;//不能用 pp = &ppage_temp
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
    ...
    /* Step 1: Get the corresponding page directory entry and page table. */
    pgdir_entryp = pgdir + PDX(va);
    pgtable = (Pte *)KADDR(PTE_ADDR(*pgdir_entryp));//要转化为虚拟地址才能连续访问
    ...
    if( !((*pgdir_entryp) & PTE_V)) {
        if(create == 1) {
            ...
            ppage -> pp_ref ++; //增加引用次数
            /*ppage 是一个struct Page类型指针，并不代表一个具体页面地址*/
            pgtable = (Pte *)page2pa(ppage); // 把page转化为物理地址
            //(*pgdir_entryp) &= 0xFFF;
            (*pgdir_entryp) = (Pte)pgtable;
            (*pgdir_entryp) |= PTE_V|PTE_R;
            pgtable = (Pte *)KADDR((u_long)pgtable); //还要转化为虚拟地址 
        }else{
            *ppte = NULL;//失败需要把指针设为NULL
            return -E_NO_MEM;
        }
    }
...
page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
    ...
    /*首先要找到虚拟地址va对应的页表项指针pgtable_entry*/
    pgdir_walk(pgdir, va, 0, &pgtable_entry);

    if (pgtable_entry != 0 && (*pgtable_entry & PTE_V) != 0) {
        if (pa2page(*pgtable_entry) != pp) {
            page_remove(pgdir, va); //里面已经包含tlb_invalidate(pgdir, va);
        } else	{
